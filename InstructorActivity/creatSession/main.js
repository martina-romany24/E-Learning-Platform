...
document.getElementById("sessionForm").addEventListener("submit", function(event) {
    ...
    let session = {
        ...
        url: document.getElementById("sessionUrl").value
    };

    let sessions = JSON.parse(localStorage.getItem("sessions")) || [];
    sessions.push(session);

    if (!loggedTeacher.InstructorSessions) {
        loggedTeacher.InstructorSessions = [];
    }
    loggedTeacher.InstructorSessions.push(session);

    // Save updated loggedTeacher to localStorage
    localStorage.setItem("loggedTeacher", JSON.stringify(loggedTeacher));

    // If still want to store sessions separately
