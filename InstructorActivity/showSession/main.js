...
function loadSessions() {
    ...
    if (!loggedTeacher || !Array.isArray(loggedTeacher.InstructorSessions)) {
        document.getElementById("sessionTableBody").innerHTML = `<tr><td style="text-align:center; padding-top:10px;" colspan='5'>No sessions found</td></tr>`;
        return;
    }

    let sessions = loggedTeacher.InstructorSessions;
    let tableBody = document.getElementById("sessionTableBody");
    tableBody.innerHTML = "";

    if (sessions.length === 0) {
        tableBody.innerHTML = `<tr><td style="text-align:center; padding-top:10px;" colspan='5'>No sessions found</td></tr>`;
    } else {
        sessions.forEach((session, index) => {
            let row = `
                <tr>
                    ...
                    <td><button class="view" onclick="window.open('${session.url}', '_blank')">View</button></td>
                    <td><button class="btn delete" onclick="deleteSession(${index})">Delete</button></td>
                </tr>
            `;
...
function deleteSession(index) {
    ...
    if (!loggedTeacher || !Array.isArray(loggedTeacher.InstructorSessions)) return;

    loggedTeacher.InstructorSessions.splice(index, 1);
